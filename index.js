...
async function run() {
  try {
    ...
    const foodCollection = client.db("begunVajji").collection("foods");
    const orderCollection = client.db("begunVajji").collection("orders");
    const blogCollection = client.db("begunVajji").collection("blogs");
    // json web token
...
app.get("/", (req, res) => {
  res.send("Begun Vajji Restaurant server Site is running");
});
app.listen(port, () => {
